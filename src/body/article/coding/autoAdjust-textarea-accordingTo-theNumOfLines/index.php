    ...
    <main>
      <div class="h1_c">
        ...
        <ol class="tableOfContents" aria-label="目次">
          ...
          <li><a href="#description" class="tableOfContents-link">コードの説明</a></li>
          <li><a href="#s3" class="tableOfContents-link">サンプル</a></li>
          <li><a href="#s4" class="tableOfContents-link">注意</a></li>
          <li><a href="#s5" class="tableOfContents-link">最後に</a></li>
        </ol>
      </div>

      <section id="code">
        <h2>コード</h2>
        <p class="p2">最低限のコードはこちらになります。</p>
        <pre class="p2"><code class="blockCode_c"><span class="code-line">&lt;<span class="code-pink">textarea</span> <span class="code-green">class</span>=<span class="code-yellow">"sample"</span>&gt;&lt;/<span class="code-pink">textarea</span>&gt;</span></code></pre>
        <pre class="p2"><code class="blockCode_c"><span class="code-line"><span class="code-blue">const</span> <var class="code-purple">tareaE</var> <span class="code-pink">=</span> document.<span class="code-blue">querySelector</span>(<span class="code-yellow">'.sample'</span>);</span>
...
<span class="code-line">});</span></code></pre>
      ...
      <section id="description">
        <h2>コードの説明</h2>
        <div class="p1">
          <p class="p2">後々<code class="inlineCode">textarea</code>要素（以下「テキストエリア」）に対してイベントを設定したりCSSを指定したりするために、まずは<code class="inlineCode">querySelector()</code>メソッドを使ってテキストエリアを取得します（<a href="https://developer.mozilla.org/ja/docs/Web/API/Document/querySelector">querySelector()メソッドについて詳しく知る</a>）。</p>
          <pre class="p2"><code class="blockCode_c"><span class="code-line"><span class="code-blue">const</span> <var class="code-purple fs_n">tareaE</var> <span class="code-pink">=</span> document.<span class="code-blue">querySelector</span>(<span class="code-yellow">'.sample'</span>);</span>
...
<span class="code-line code-gray">// });</span></code></pre>
        ...
        <div class="p1">
          <p class="p2">次に、テキストエリアに<code class="inlineCode">input</code>イベントを指定します。これによりテキストエリアに文字が入力されるたびにイベントが発火します（<a href="https://developer.mozilla.org/ja/docs/Web/API/HTMLElement/input_event">inputイベントについて詳しく知る</a>）。</p>
          <pre class="p2"><code class="blockCode_c"><span class="code-line code-gray">// const <var class="fs_n">tareaE</var> = document.querySelector('.sample');</span>
<span class="code-line code-gray">// const <var>bdw</var> = 4;</span>
<span class="code-line">tareaE.<span class="code-blue">addEventListener</span>(<span class="code-yellow">'input'</span>, () <span class="code-blue">=&gt;</span> {</span>
<span class="code-line code-gray">//   tareaE.style.height = "20px";</span>
<span class="code-line code-gray">//   tareaE.style.height = tareaE.scrollHeight + bdw + "px";</span>
...
<span class="code-line code-gray">// });</span></code></pre>
        ...
        <div class="p1">
          <p class="p2">境界線の太さをJSで取得する場合は下のようなコードを書きます。</p>
          <pre class="p2"><code class="blockCode_c"><span class="code-line"><span class="code-blue">const</span> <var class="code-purple">compStyles</var> <span class="code-pink">=</span> <span class="code-green">getComputedStyle</span>(tareaE, <span class="code-purple">null</span>);</span>
...
<span class="code-line"><span class="code-blue">const</span> <var class="code-purple">bdbW</var> <span class="code-pink">=</span> compStyles.<span class="code-green">getPropertyValue</span>(<span class="code-yellow">"border-bottom-width"</span>);</span></span></code></pre>
        </div>
        <p class="p1">ではなぜ「20px」も指定しているのでしょうか？</p>
        <div class="p1">
          <p class="p2">それを指定しないと複数行にわたる文字を一度に消したときに高さが縮んでくれません。また、文字を1文字ずつ消したときにおかしな動作をします。試してみよう！</p>
          <div class="textarea_c p2">
        ...
        <p class="p1">この問題を解決するために20pxを指定します。<br>ではなぜ「20px」という値にしているのでしょうか？</p>
        <p class="p1">結論から言うと、20pxである必要はありません。</p>
        <p class="p1">テキストエリアの<code class="inlineCode">height</code>プロパティに<code class="inlineCode">scrollHeight</code>の値を指定する前に、十分に小さい値を指定します。その時、<code class="inlineCode">scrollHeight</code>の値は、要素の中身をスクロールなしで見るための最小の値になっています。そこで、テキストエリアの<code class="inlineCode">height</code>プロパティに<code class="inlineCode">scrollHeight</code>の値を指定することで、テキストエリアはちょうどいい大きさになるわけです。</p>
        <p class="p1">「十分に小さい値」を説明するのが難しいです...。もし分からなかったら「1px」と指定しておけば大丈夫です。</p>
      </section>

      <section id="s3">
        <h2>サンプル</h2>
        <div class="p1 mb24rem">
          <p class="p2">下のサンプルは次のような特徴を持っています。</p>
          <ul class="list_ml p2">
            <li class="listE"><code class="inlineCode">min-height: 100px;</code>を指定してデフォルトで数行分の高さを持たせています。</li>
            <li class="listE"><code class="inlineCode">max-height: 200px;</code>を指定して高さの最大値を指定しています。</li>
            <li class="listE">変数<var class="inlineCode">bdw</var>にはテキストエリアの上下境界線の太さの合計値に+1した値を代入しています。そうしないとテキストエリアの高さが100px～200pxのときに垂直スクロールバーが表示されてしまいました。</li>
          </ul>
        </div>
      ...
      <section id="s4">
        <h2>注意</h2>
        <p class="p1">もし<code class="inlineCode">height</code>プロパティにトランジションエフェクトを適用させていると、複数行を消したときの動作がおかしくなります（以下のサンプルを参照）。したがって、テキストエリアに<code class="inlineCode">transition</code>プロパティを指定したい場合は、<code class="inlineCode">transition-property</code>プロパティの値に<code class="inlineCode">all</code>と<code class="inlineCode">height</code>以外を設定してください。</p>
